...
const cargoCommand = process.env.HOPPER_MCP_DISTRIBUTION_CARGO || "cargo";
const npmCommand = process.env.HOPPER_MCP_DISTRIBUTION_NPM || "npm";
...
async function main() {
  try {
    ...
    await run("packageRelease", npmCommand, ["run", "package:release"]);
    return {
      ok: true,
      phase: "complete",
    };
