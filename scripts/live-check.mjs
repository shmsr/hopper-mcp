...
const cargoCommand = process.env.HOPPER_MCP_LIVE_CHECK_CARGO || "cargo";
const npmCommand = process.env.HOPPER_MCP_LIVE_CHECK_NPM || "npm";
const nodeCommand = process.env.HOPPER_MCP_LIVE_CHECK_NODE || "node";
const cleanupCommands = cleanupCommandsFromEnv({
  pkillVar: "HOPPER_MCP_LIVE_CHECK_PKILL",
  pgrepVar: "HOPPER_MCP_LIVE_CHECK_PGREP",
  launchctlVar: "HOPPER_MCP_LIVE_CHECK_LAUNCHCTL",
});
...
async function main() {
  ...
  try {
    ...
    await run("testLiveJs", nodeCommand, ["--test", "test/live.mjs"], { HOPPER_MCP_LIVE: "1" });
    await run("testLiveRust", cargoCommand, [
      "test",
      "-p",
      "hopper-mcpd",
      "--test",
      "live_bridge_contract",
      "daemon_default_live_bridge_ingests_echo_when_enabled",
    ], { HOPPER_MCP_LIVE: "1" });
    await cleanupRunnerState();
    await run("liveCorpus", nodeCommand, ["scripts/live-corpus.mjs"], { HOPPER_MCP_LIVE: "1" });
    result = {
      ok: true,
      phase: "complete",
    };
