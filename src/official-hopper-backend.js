...
export class OfficialHopperBackend {
  ...
  async callInternalTool(name, args = {}, { validate = true } = {}) {
    return this.#enqueueCall(() => this.#callToolUnlocked(name, args, {
  ...
  async #callToolUnlocked(name, args = {}, { confirmLiveWrite = false, validate = true, bypassWriteGate = false } = {}) {
    if (this.isWriteTool(name) && !bypassWriteGate && (!this.enableWrites || !confirmLiveWrite)) {
  ...
  async #start() {
    ...
    this.child.on("exit", (code, signal) => {
      this.initialized = false;
      this.tools = null;
      // Reset the call queue: the previous chain may still be holding a
      // rejected tail, which would propagate to the next #enqueueCall after
      // restart and surface as an out-of-context error from the prior crash.
      this.callQueue = Promise.resolve();
  ...
  close() {
    ...
    this.initialized = false;
    this.tools = null;
    // The exit handler also resets callQueue, but exit fires async; reset
    // here too so a synchronous start() after close() doesn't see a stale
    // rejected tail.
    this.callQueue = Promise.resolve();
    const pending = Array.from(this.pending.values());
    this.pending.clear();
