...
export async function buildOfficialSnapshot(backend, {
  ...
  documentName: preferredDocumentName = null,
} = {}) {
  const documentName = await resolveDocumentName(backend, preferredDocumentName);
  if (!documentName) {
...
async function resolveDocumentName(backend, preferredDocumentName = null) {
  if (typeof preferredDocumentName === "string" && preferredDocumentName.trim()) {
    return preferredDocumentName;
  }

  const currentDocument = await officialPayload(backend, "current_document");
  if (typeof currentDocument === "string" && currentDocument.trim()) {
    return currentDocument;
  }

  const documents = await optionalOfficialPayload(backend, "list_documents", {}, []);
  if (!Array.isArray(documents) || documents.length === 0) return null;

  if (currentDocument != null) {
    const coercedCurrent = String(currentDocument);
    const exactMatch = documents.find((document) => String(document) === coercedCurrent);
    if (typeof exactMatch === "string" && exactMatch.trim()) return exactMatch;
  }

  const firstNamedDocument = documents.find((document) => typeof document === "string" && document.trim());
  return firstNamedDocument ?? null;
}
