...
export function registerResources(server, store) {
  ...
  // RFC 6570 form-style query expansion: `{?radius}` produces "?radius=value"
  // when bound and disappears when not. The previous `?radius={radius}` form
  // is a literal `?` followed by a level-1 expansion, which a strict client
  // resolves to `?radius=` (empty) when radius is omitted — losing the
  // optional-query intent.
  server.registerResource(
    "graph_callers",
    new ResourceTemplate("hopper://graph/callers/{addr}{?radius}", { list: undefined }),
    {
      title: "Caller Graph",
  ...
  server.registerResource(
    "graph_callees",
    new ResourceTemplate("hopper://graph/callees/{addr}{?radius}", { list: undefined }),
    {
      title: "Callee Graph",
